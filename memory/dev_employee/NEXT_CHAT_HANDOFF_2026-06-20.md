...
Paste the startup prompt in this file into a new conversation. The new conversation must read GitHub durable context before planning or execution.
...
`audit_current_main_then_diagnose_effective_tool_surface`

## Current runtime truth

- OpenClaw native UI remains the commercial primary UI.
- Plugin `oris-dev-employee` `0.1.0` is installed.
- The three approved read-only tools and three typed hooks are registered and pass direct runtime checks.
- The runtime is currently restored to the exact healthy tools-denied baseline.
- No write tool is authorized.
- No product task is active.
- Production host `8.136.28.6` is out of scope.
...
The candidate policy, Gateway restart, Skill visibility, plugin inventory and direct ORIS calls passed. Three native Agent turns completed through Gateway in one persisted session, but telemetry recorded:

- `model_call_ended=3`;
- `agent_end=3`;
- `after_tool_call=0`;
- no approved tool name observed.

Rollback restored the exact tools-denied baseline and healthy Gateway.

The unresolved boundary is whether the three optional ORIS tools were absent from the model-facing effective tool inventory or were present but the selected runtime provider/model did not call them.
...
The diagnostic uses native `tools.effective`, runs no model turn, invokes no ORIS tool and always rolls back.

## Mandatory code-first gate

Before running the diagnostic, the new conversation must follow:

`docs/DEV_EMPLOYEE_CODE_FIRST_CONTINUATION_GATE_2026-06-20.md`

The new conversation must inspect current `main` for duplicate functions, variables, classes, parsers, policies, service helpers, rollback helpers, evidence publishers and entrypoints; competing authorities; import cycles; hardcoding; oversized modules; stale paths and contract errors.

No runtime action is allowed until the latest commit passes `CODE_AUDIT_PASS` with all tracked findings at zero.

## Required read order

1. `memory/dev_employee/CONTEXT_INDEX.md`
2. `memory/dev_employee/CURRENT_STATE_2026-06-20.md`
3. `memory/dev_employee/SESSION_ARCHIVE_2026-06-20.md`
4. `memory/dev_employee/ENVIRONMENT_AND_WORKING_CONTEXT_2026-06-20.md`
...
9. latest evidence commit `d5cea6980ad46a51cb4f26f8e6229c11539ea2d5`
10. `docs/DEV_EMPLOYEE_ENVIRONMENT_AND_OPERATING_MODEL_ADDENDUM_2026-06-19.md`
11. `docs/DEV_EMPLOYEE_ENGINEERING_STANDARD_2026-06-16.md`
12. `docs/DEV_EMPLOYEE_ENGINEERING_STANDARD_ADDENDUM_2026-06-17.md`
13. `docs/DEV_EMPLOYEE_ENGINEERING_STANDARD_ADDENDUM_2026-06-19.md`
...
- 当前架构；
- 已完成能力；
- 插件、Skill、typed tools 和 hooks 状态；
...
第一优先级不是继续调 runtime，而是先修正代码问题。

必须先对当前 `main` 做完整 code-first audit，重点检查并修复：
...
10. config/code separation and contract errors。

不要通过 scanner allowlist 隐藏真实问题。一个规则只保留一个权威实现。大文件按 policy、runtime adapter、service control、validation、evidence、entrypoint 分层解耦。

在得到以下结果前，禁止访问 OpenClaw runtime、禁止重启 Gateway、禁止运行 `tools.effective`、禁止模型调用、禁止提交产品任务：
...
- 使用已验证的 persisted Agent session context；
- 不运行 model turn；
- 不调用 ORIS tool；
- 不提交产品任务；
- 不增加 write tool；
- 只记录脱敏的 approved-tool presence/count/ownership；
- 不记录 raw RPC、无关工具名、tool description、raw session id、prompt、tool args/results、secret 或 raw config；
...
- RPC 不可用或不安全：停止并修复 diagnostic path，不得用 direct invocation 或 catalog inventory 替代。

当前不可逆产品决策：

1. `https://control.orisfy.com` 主界面是 OpenClaw 原生 UI 和原生会话体系；
2. 自研 ORIS Web Console 仅保留受限 `/admin` 和回滚/诊断路径；
3. 正式链路为：用户 → OpenClaw 原生 UI → 原生 ORIS Plugin / Agent Harness → ORIS 任务治理 → Codex 真实代码执行 → 产品提交、测试和 ORIS evidence 返回 OpenClaw；
4. 不重新安装或升级 OpenClaw；
5. 复用 `openclaw-gateway.service` 和 `127.0.0.1:18789`；
...
- 开发/控制/执行：`43.106.55.255`，用户 `admin`；
...
- 生产：`8.136.28.6`，用户 `deploy`，不要触碰；
- OpenClaw：`2026.5.19 (a185ca2)`；
- Gateway：`127.0.0.1:18789`；
- enqueue/status：`127.0.0.1:18891`；
- Intake：`127.0.0.1:18892`；
- Web Console：`127.0.0.1:18893`；
- PostgreSQL：`oris_insight` / schema `insight`，只用于 research/insight，与 Dev Employee task state 分离；
...
- Summary、GitHub 和聊天不得输出 Token、密码、密钥、raw config、raw session id、conversation content 或 private marker；
...
5. P0 后设计带 approval、RBAC、idempotency、audit 的 typed write actions；
