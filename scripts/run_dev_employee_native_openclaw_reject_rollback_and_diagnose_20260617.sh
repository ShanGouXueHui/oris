...
TARGET="/home/admin/projects/oris/scripts/dev_employee_native_openclaw_reject_rollback_and_diagnose_20260617.sh"
...
if [ ! -f "$TARGET" ]; then
  summary_failed "diagnosis_entry_missing" "PULL_ORIS_MAIN_AND_RETRY"
  exit 1
fi

if ! bash -n "$TARGET" >/dev/null 2>&1; then
  summary_failed "diagnosis_entry_syntax_invalid" "REPAIR_DIAGNOSIS_ENTRY"
  exit 1
fi
