...
WEB_PAGE="/tmp/oris-queue-lifecycle-web-page-$STAMP.html"
GIT_OUTPUT="/tmp/oris-queue-lifecycle-git-$STAMP.log"
...
payload = {
  ...
  "local_stash_restore": "$LOCAL_STASH_RESTORE",
  "services": {
...
commit_logs() {
  ...
  git commit --only -m "test(dev-employee): deploy queue lifecycle hardening $STAMP" -- "${files[@]}" > "$GIT_OUTPUT" 2>&1 || {
...
systemctl --user show "$BRIDGE_SERVICE" -p ExecStart --value | grep -q 'dev_employee_supervised_bridge_v3.py' || fail "bridge_override_not_effective" "INSPECT_BRIDGE_OVERRIDE"
systemctl --user show "$INTAKE_SERVICE" -p ExecStart --value | grep -q 'dev_employee_intake_api_v2.py' || fail "intake_override_not_effective" "INSPECT_INTAKE_OVERRIDE"
systemctl --user show "$WEB_SERVICE" -p ExecStart --value | grep -q 'dev_employee_web_console_v2.py' || fail "web_override_not_effective" "INSPECT_WEB_OVERRIDE"
SERVICE_SWITCH="PASS"
...
python3 - <<'PY' > "$STALE_RECOVERY" 2>> "$RUN_LOG"
import json
from scripts.dev_employee_recover_stale_tasks import recover
print(json.dumps(recover(120), ensure_ascii=False, indent=2))
PY
[ "$?" -eq 0 ] || fail "stale_recovery_failed" "INSPECT_STALE_RECOVERY"
...
rm -f "$WEB_PAGE" "$GIT_OUTPUT" /tmp/oris-queue-lifecycle-unauth-$STAMP.json /tmp/oris-queue-lifecycle-bridge-journal-$STAMP.log
if [ "$RESULT" = "PASS" ]; then
  exit 0
fi
exit 1
