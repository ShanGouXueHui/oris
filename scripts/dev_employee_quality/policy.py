...
def load_policy(repo_root: Path, policy_path: Path | None = None) -> ScanPolicy:
    ...
    return ScanPolicy(
        ...
        excluded_path_prefixes=tuple(data["excluded_path_prefixes"]),
        excluded_file_names=frozenset(data["excluded_file_names"]),
        exclude_timestamped_json=bool(data["exclude_timestamped_json"]),
        source_extensions=frozenset(data["source_extensions"]),
        code_extensions=frozenset(data["code_extensions"]),
