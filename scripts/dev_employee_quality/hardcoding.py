...
_COMMERCIAL_TASK_PREFIX = "commercial" + "-"
...
def scan(source: SourceFile, policy: ScanPolicy) -> list[Finding]:
    ...
    endpoint_values = tuple(
        f"127.0.0.1:{port}" for port in policy.environment_ports
    )
        ...
        if "environment_loopback_port" in policy.detectors:
            for value in endpoint_values:
                if value in line:
                    findings.append(
                        Finding(
                            "environment_loopback_port",
                            source.relative_path,
                            line_number,
                            "derive endpoints from runtime configuration",
                            value,
                        )
                    )
                ...
                if value in line:
                    findings.append(
                        Finding(
                            ...
                            source.relative_path,
                            line_number,
                            "derive public domains from deployment configuration",
                            value,
                        )
                    )
                ...
                if value in line:
                    findings.append(
                        Finding(
                            ...
                            source.relative_path,
                            line_number,
                            "shared source must not embed acceptance project identities",
                            value,
                        )
                    )
                ...
                if marker in line:
                    findings.append(
                        Finding(
                            ...
                            source.relative_path,
                            line_number,
                            "derive host paths from registry or runtime context",
                            marker,
                        )
                    )
        if "embedded_commit_sha" in policy.detectors:
            tokens = stripped.replace('"', " ").replace("'", " ").replace("=", " ")
                ...
                if _is_hex_identifier(candidate):
                    findings.append(
                        Finding(
                            ...
                            source.relative_path,
                            line_number,
                            "discover commit identifiers from task metadata or evidence",
                            candidate,
                        )
                    )
        if (
            "embedded_commercial_task_id" in policy.detectors
            and _COMMERCIAL_TASK_PREFIX in line
            and "20" in line
        ):
            findings.append(
                Finding(
                    "embedded_commercial_task_id",
                    source.relative_path,
                    line_number,
                    "read task identifiers from the current task descriptor",
                )
            )
