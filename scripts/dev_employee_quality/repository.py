...
def _source_file(repo_root: Path, relative: Path) -> SourceFile:
    ...
    for root_name in policy.scan_roots:
        ...
        for path in sorted(scan_root.rglob("*")):
            ...
            if not _is_excluded(relative, policy):
                yield _source_file(repo_root, relative)


def iter_selected_source_files(
    repo_root: Path,
    policy: ScanPolicy,
...
def scan_repository(
    ...
    sources = (
        iter_source_files(repo_root, policy)
        if relative_paths is None
        else iter_selected_source_files(repo_root, policy, relative_paths)
    )
    return _scan_sources(sources, policy)
