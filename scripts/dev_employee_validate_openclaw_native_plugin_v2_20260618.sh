...
TMP_ROOT="$(mktemp -d /tmp/oris-openclaw-plugin-validation-v2-${STAMP}-XXXXXX)"
...
old = '[ -z "$ORIS_STATUS_BEFORE" ] || fail_now "oris_main_worktree_not_clean_before_validation" "PRESERVE_ORIS_MAIN_WORKTREE_BEFORE_VALIDATION"'
new = '''if [ -z "$ORIS_STATUS_BEFORE" ]; then
