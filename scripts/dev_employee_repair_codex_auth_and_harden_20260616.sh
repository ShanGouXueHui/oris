...
commit_logs() {
  ...
  git commit -m "test(dev-employee): record Codex auth preflight $STAMP" > "$TMP_OUTPUT" 2>&1
  if [ "$?" -ne 0 ]; then
...
print_summary() {
  BRIDGE_STATE="$(systemctl --user is-active "$BRIDGE_SERVICE" 2>/dev/null || true)"
  INTAKE_STATE="$(systemctl --user is-active "$INTAKE_SERVICE" 2>/dev/null || true)"
  WEB_STATE="$(systemctl --user is-active "$WEB_SERVICE" 2>/dev/null || true)"
  echo
  echo "===== SUMMARY ====="
  echo "RESULT=$RESULT"
  ...
  echo "BRIDGE_SERVICE=$BRIDGE_STATE"
  echo "INTAKE_SERVICE=$INTAKE_STATE"
  echo "WEB_CONSOLE_SERVICE=$WEB_STATE"
...
python3 -m pytest -q \
  ...
  tests/test_dev_employee_codex_auth_preflight.py >> "$RUN_LOG" 2>&1
if [ "$?" -ne 0 ]; then
...
if [ "$ADMIN_RC" -ne 0 ]; then
  ...
  echo
  echo "Codex 登录已失效。下面会在当前 admin 终端执行重新登录；登录内容不会写入 GitHub 日志。"
  ...
  if [ "$LOGIN_RC" -ne 0 ]; then
    ...
    fail_and_finish "COMPLETE_CODEX_LOGIN_AND_RERUN_THIS_SCRIPT"
  fi
fi
