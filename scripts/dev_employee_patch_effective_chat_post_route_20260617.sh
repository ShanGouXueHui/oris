...
PATCHER_SELF_TEST="NOT_RUN"
CONFIG_BACKUP="NOT_RUN"
METHOD_POLICY_PATCH="NOT_RUN"
LOCATION_GUARDS_PRESERVED="NOT_RUN"
...
payload={
  ...
  "effective_config_verified":"$EFFECTIVE_CONFIG_VERIFIED",
  "patcher_self_test":"$PATCHER_SELF_TEST",
  "config_path":"$CONFIG",
  "config_backup":"$CONFIG_BACKUP",
  "backup_sha256":"$BACKUP_SHA256" or None,
  "patched_sha256":"$PATCHED_SHA256" or None,
  "method_policy_patch":"$METHOD_POLICY_PATCH",
  "location_guards_preserved":"$LOCATION_GUARDS_PRESERVED",
  "exact_chat_location":"$EXACT_CHAT_LOCATION",
  "nginx_config_test":"$NGINX_CONFIG_TEST",
  "nginx_reload":"$NGINX_RELOAD",
  "public_get_auth_gate":"$PUBLIC_GET_AUTH_GATE",
  "public_chat_post_auth_gate":"$PUBLIC_CHAT_POST_AUTH_GATE",
  "public_other_post_blocked":"$PUBLIC_OTHER_POST_BLOCKED",
  "local_tls_chat_post_auth_gate":"$LOCAL_TLS_CHAT_POST_AUTH_GATE",
...
commit_logs() {
  local files=()
  local file
  for file in "$RUN_LOG" "$EVIDENCE_JSON" "$PATCH_AUDIT" "$DIRECT_SMOKE"; do
    [ -f "$file" ] && files+=("${file#$ORIS/}")
  done
  [ "${#files[@]}" -gt 0 ] || { LOG_COMMIT="NO_LOG_FILES"; return 1; }
...
summary() {
  ...
  echo "PATCHER_SELF_TEST=$PATCHER_SELF_TEST"
  echo "CONFIG_BACKUP=$CONFIG_BACKUP"
  echo "METHOD_POLICY_PATCH=$METHOD_POLICY_PATCH"
  echo "LOCATION_GUARDS_PRESERVED=$LOCATION_GUARDS_PRESERVED"
...
fail() {
  ...
  commit_logs || true
  summary
  rm -f "$PATCHER" "$PATCHED" "$GIT_OUTPUT"
  exit 1
}
...
git show origin/main:scripts/dev_employee_patch_nginx_chat_route.py > "$PATCHER" || fail "patcher_materialize_failed" "RESTORE_STRUCTURAL_PATCHER"
PYTHONDONTWRITEBYTECODE=1 /usr/bin/python3 "$PATCHER" --self-test >> "$RUN_LOG" 2>&1 || fail "patcher_self_test_failed" "FIX_STRUCTURAL_PATCHER"
PATCHER_SELF_TEST="PASS"
...
data=json.dumps({'session_id':sid,'message':'帮助'},ensure_ascii=False).encode('utf-8')
...
for path in [
    root/'orchestration/dev_employee_chat_sessions'/('%s.json'%sid),
    root/'run/dev_employee_chat_locks'/('%s.lock'%sid),
]:
    if path.exists():
        path.unlink()
...
PYTHONDONTWRITEBYTECODE=1 /usr/bin/python3 "$PATCHER" \
  --input "$CONFIG" \
  --output "$PATCHED" \
  --audit "$PATCH_AUDIT" >> "$RUN_LOG" 2>&1 || fail "effective_config_patch_generation_failed" "INSPECT_EFFECTIVE_NGINX_CONFIG"

python3 - "$PATCH_AUDIT" <<'PY' >> "$RUN_LOG" 2>&1
import json
import sys
payload=json.load(open(sys.argv[1],encoding='utf-8'))
assert payload['server_guard_safe'] is True
assert payload['exact_chat_location_present'] is True
assert payload['other_write_routes_allowed'] is False
assert payload['location_method_guard_count_after'] == payload['location_method_guard_count_before']
PY
[ "$?" -eq 0 ] || fail "patch_audit_failed" "INSPECT_PATCH_AUDIT"
METHOD_POLICY_PATCH="PASS"
LOCATION_GUARDS_PRESERVED="PASS"
EXACT_CHAT_LOCATION="PASS"
...
log "PUBLIC_GET_HTTP=$GET_CODE"
log "PUBLIC_CHAT_POST_HTTP=$CHAT_CODE"
log "PUBLIC_OTHER_POST_HTTP=$OTHER_CODE"
log "LOCAL_TLS_CHAT_POST_HTTP=$LOCAL_CHAT_CODE"
...
commit_logs || {
  RESULT="FAILED"
  FAILURE_CODE="patch_evidence_push_failed"
  NEXT_ACTION="RESOLVE_PATCH_EVIDENCE_PUSH"
}
summary
rm -f "$PATCHER" "$PATCHED" "$GIT_OUTPUT"
[ "$RESULT" = "PASS" ] && exit 0
