#!/usr/bin/env python3
"""Safe v2 patch runner for Web Console submit audit logging.

This replaces the earlier audit patch runner whose embedded template expanded
`\n` incorrectly while writing the target Python source.
"""
...
def patch_console() -> bool:
    ...
    if "from datetime import datetime, timezone" not in text:
        ...
        changed = True
    if "WEB_CONSOLE_AUDIT_DIR" not in text:
        text = text.replace(
        ...
        changed = True
    if "def write_audit_event(" not in text:
        marker = "def load_env(path: Path) -> dict[str, str]:\n"
        audit_helpers = r'''def now_iso() -> str:
