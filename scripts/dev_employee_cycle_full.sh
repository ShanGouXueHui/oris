...
echo "FULL_CYCLE_REF=${HEAD_SHORT} logs/dev_employee/latest_task_list.json logs/dev_employee/latest_task_status.json logs/dev_employee/latest_task_binding.json logs/dev_employee/latest_commercial_readiness.json"
echo "FULL_CYCLE_RESULT={\"cycle_rc\":${CYCLE_RC},\"task_list_rc\":${TASK_LIST_RC},\"task_status_rc\":${TASK_STATUS_RC},\"binding_rc\":${BINDING_RC},\"readiness_rc\":${READINESS_RC},\"commit_rc\":${COMMIT_RC},\"push_rc\":${PUSH_RC}}"

if [ "$CYCLE_RC" -eq 0 ] && [ "$TASK_LIST_RC" -eq 0 ] && [ "$TASK_STATUS_RC" -eq 0 ] && [ "$BINDING_RC" -eq 0 ] && [ "$READINESS_RC" -eq 0 ] && [ "$COMMIT_RC" -eq 0 ] && [ "$PUSH_RC" -eq 0 ]; then
  exit 0
fi
exit 1
