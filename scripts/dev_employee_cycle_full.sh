...
if ! git diff --cached --quiet; then
  git commit -m "logs(dev-employee): update commercial readiness artifacts" >/tmp/oris_dev_employee_cycle_full_git.log 2>&1
  COMMIT_RC=$?
...
echo "FULL_CYCLE_REF=${HEAD_SHORT} logs/dev_employee/latest_commercial_readiness.json logs/dev_employee/latest_commercial_readiness.md"
echo "FULL_CYCLE_RESULT={\"cycle_rc\":${CYCLE_RC},\"readiness_rc\":${READINESS_RC},\"commit_rc\":${COMMIT_RC},\"push_rc\":${PUSH_RC}}"

if [ "$CYCLE_RC" -eq 0 ] && [ "$READINESS_RC" -eq 0 ] && [ "$COMMIT_RC" -eq 0 ] && [ "$PUSH_RC" -eq 0 ]; then
  exit 0
fi
exit 1
