...
def candidate_policy_compatibility(
    ...
    allow = _optional_string_list(tools, "allow")
    also_allow = _optional_string_list(tools, "alsoAllow")
    deny = _optional_string_list(tools, "deny")
    if allow is None or also_allow is None or deny is None:
    ...
    allow_active = bool(allow)
    also_allow_active = bool(also_allow)
    single_scope = not (allow_active and also_allow_active)
    authorized = set(allow if allow_active else also_allow)
    authorization_scope = "allow" if allow_active else "profile-plus-alsoAllow"
    ...
    checks = {
        ...
        "single_authorization_scope": single_scope,
        "authorization_scope_present": allow_active or also_allow_active,
        "approved_authorized": approved.issubset(authorized),
