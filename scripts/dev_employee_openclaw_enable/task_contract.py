...
def _evidence_target(evidence: dict[str, Any], name: str) -> dict[str, str]:
    ...
    return {
        ...
        "commit_message_prefix": require_string(
            ...
            f"evidence.{name}.commit_message_prefix",
        ),
    }
...
def load_runtime_contract(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(raw_turns, list) or not all(
        isinstance(item, dict) for item in raw_turns
    ):
        raise RuntimeError("invalid agent_acceptance.turns")
    turns = tuple(dict(item) for item in raw_turns)
    expected_tools = tuple(
        require_string(item.get("expected_tool"), "agent_acceptance.turn.expected_tool")
        for item in turns
    )
    if len(expected_tools) != len(set(expected_tools)) or set(expected_tools) != set(
        approved_tools
    ):
