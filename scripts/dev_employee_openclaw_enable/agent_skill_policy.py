...
class AgentSkillPolicyChange:
    ...
    def mode(self) -> str:
        ...
        return (
            f"skill-added-to-{self.scope}"
            if self.changed
            else f"skill-already-visible-via-{self.scope}"
        )
...
def ensure_skill_visible(
    ...
    if entry is not None and "skills" in entry:
        values = unique_string_list(entry["skills"], f"agent {agent_id} skills")
        before = len(values)
    ...
    if isinstance(defaults, dict) and "skills" in defaults:
        values = unique_string_list(defaults["skills"], "agents.defaults.skills")
        before = len(values)
        changed = skill_name not in values
        if changed:
            values.append(skill_name)
...
def skill_is_visible(config: dict[str, Any], skill_name: str, agent_id: str) -> bool:
    ...
    entry = _find(agents, agent_id)
    if entry is not None and "skills" in entry:
        return skill_name in unique_string_list(
            entry["skills"],
